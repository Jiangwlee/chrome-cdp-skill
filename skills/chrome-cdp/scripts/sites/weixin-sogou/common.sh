#!/usr/bin/env bash
# This file provides shared helpers for the chrome-cdp weixin.sogou.com workflows.
...
#
# sogou_find_target prefers an existing weixin.sogou.com/weixin tab, then any
# weixin.sogou.com tab, then falls back to the first available page tab.
# This lets the search script run without requiring a pre-opened Sogou tab.
...
sogou_find_target() {
  ...
  jq -r '
    ...
    | (map(select(.url | test("^https://weixin\\.sogou\\.com/weixin"))) +
       map(select(.url | test("^https://weixin\\.sogou\\.com/"))) +
       .)
    | unique_by(.targetId)
    | .[0].targetId // empty
  ' <<<"$pages"
